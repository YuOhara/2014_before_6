...
public:
  ...
  void image_cb(const sensor_msgs::ImageConstPtr& msg_ptr){
    *screen = cv::Scalar(100, 100, 100);
    ...
    show_rect();
    if(m_p.on_move==1){
      cv::rectangle(*screen, cv::Point(WIDTH/2+W_M-90, H_M-50), cv::Point(WIDTH/2+W_M+90, H_M+10), cv::Scalar(20, 200, 20), -1, CV_AA);
    }
    else if(m_p.on_move==2){
      cv::rectangle(*screen, cv::Point(WIDTH/2+W_M-90, H_M-50), cv::Point(WIDTH/2+W_M+90, H_M+10), cv::Scalar(20, 20, 200), -1, CV_AA);
    }
    else{
      cv::rectangle(*screen, cv::Point(WIDTH/2+W_M-90, H_M-50), cv::Point(WIDTH/2+W_M+90, H_M+10), cv::Scalar(200, 20, 20), -1, CV_AA);
    }
    cv::putText(*screen, "Move!", cv::Point(WIDTH/2+W_M - 80, H_M), cv::FONT_HERSHEY_PLAIN, 3.5, cv::Scalar(100, 255, 100), 2, CV_AA);
    cv::putText(*screen, "place", cv::Point(W_M/2 - 80, H_M), cv::FONT_HERSHEY_PLAIN, 3.5, cv::Scalar(100, 255, 100), 2, CV_AA);
    cv::putText(*screen, "objects", cv::Point(WIDTH+W_M*3/2-80 - 20, H_M), cv::FONT_HERSHEY_PLAIN, 3.5, cv::Scalar(100, 255, 100), 2, CV_AA);
  ...
  static void mouse_cb(int event, int x, int y, int flags, void* param = NULL){
    ...
    if(x>WIDTH/2+W_M-90&&x<WIDTH/2+W_M+90&&y>H_M-50&&y<H_M+10){
      if(m_p->on_move==0){
	m_p->on_move=1;
      }
